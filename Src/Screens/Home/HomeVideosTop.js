...
const HomeVideosTop = ({}) => {
  ...
  const [getHomeVideosTop, setGetHomeVideosTop] = useState([]);
  ...
  const _getHomeVideosTop = async () => {
    ...
    try {
      ...
      setGetHomeVideosTop(response.data?.videos);
    } catch (err) {
 ...
 ToastCom({
  ...
  return (
    <View style={styles.container}>
      <HomeSeeAllCom
       Title={'Today New Latest Videos'}
      TitlePara={' Best of the today video list update.'}
        onPress={() => navigation.navigate(ScreensName.HOMESEEALLTOP)}
      />
      <FlatList
        horizontal
 ...
 removeClippedSubviews={false} // <- Add This
        refreshControl={
          <RefreshControl refreshing={isRefreshing} onRefresh={getHomeVideosTop} />
        }
      />
    </View>
  );
